  ...
  return (
    <section id="awards" className="bg-[#2f9ddc] min-h-screen flex items-center justify-center py-20">
      <div className="max-w-6xl w-full px-8">
        <h2 className="font-['Alexandria:Bold',sans-serif] font-bold text-white text-[48px] tracking-[-0.96px] mb-12 text-center">
          AWARDS
        </h2>
        <div className="space-y-4">
          {awards.map((award, index) => (
            <div key={index} className="bg-white p-6 rounded-lg border-l-4 border-[#29e7d1] flex gap-4">
              <span className="font-['Alexandria:Bold',sans-serif] font-bold text-[#2f9ddc] text-[20px] tracking-[-0.4px] min-w-[60px]">
                {award.year}:
              </span>
              <p className="font-['Alexandria:Bold',sans-serif] text-[#32333c] text-[18px] tracking-[-0.36px]">
                {award.description}
              </p>
            </div>
          ))}
